...
select 
	man.ID as __MANDATOR_ID, cast(cast(year(cm.START_DATE) as nvarchar(4)) + '-' + 
	REPLICATE('0',2-LEN(month(cm.START_DATE))) + cast(month(cm.START_DATE) as nvarchar(2)) as nvarchar(7)) as MONTH,
...
group by man.ID, year(cm.START_DATE), month(cm.START_DATE)
